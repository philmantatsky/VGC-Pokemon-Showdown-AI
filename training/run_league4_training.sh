...
# League round 4 (2026-09-07): the current-meta round -- pool copies from the meta-game equilibrium (make_league4_config.py), September human clones, exploiter capped, TR rosters boosted in the opponent team pool. Round 3 (exploiter at
...
.venv/bin/python training/build_league.py --config training/league4_config.json --verify-only || {
  echo "Training refused: league-2 pool failed verification. Run training/build_league.py --config training/league4_config.json first." >&2
  exit 2
}
...
exec .venv/bin/python -m vgc_bench.train \
  ...
  --team_weights data/team_weights_regmb_league4.json \
  --results_suffix league4 \
  --num_envs 8 \
  --num_eval_workers 8 \
  --port 7700 \
  --device mps \
