...
# League round 3b: round 3 with the adversary dose cut. Round 3 (exploiter at
...
.venv/bin/python training/build_league.py --config training/league3b_config.json --verify-only || {
  echo "Training refused: league-2 pool failed verification. Run training/build_league.py --config training/league3b_config.json first." >&2
  exit 2
}
...
exec .venv/bin/python -m vgc_bench.train \
  ...
  --team_weights data/team_weights_regmb_league3b.json \
  --results_suffix league3b \
  --num_envs 8 \
  --num_eval_workers 8 \
  --port 7700 \
  --device mps \
