...
.venv/bin/python build_league.py --config league2_config.json --verify-only || {
  echo "Training refused: league-2 pool failed verification. Run build_league.py --config league2_config.json first." >&2
  exit 2
}
